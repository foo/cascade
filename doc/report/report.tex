...
\subsection{Costly cascade examples}

\subsubsection{k/2 cost cascade with at most sqrt size of components}

\subsubsection{$\Theta(k^2)$ cost cascade for $\ell$ servers}

\subsection{k log k cost of a phase hypothesis for two servers}

\maciek{TODO: verify the hypothesis for small values of k by exhaustive search. With such program we can check easily the properties of groups (Section~\ref{ss:groups})}

\subsection{Classification of cascades into cost groups (exponential)}
\label{ss:groups}

\subsection{Counting the number of subclusters and properties of function H1}
