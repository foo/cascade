...
\subsection{Cascade dependency on component sizes}

\maciek{Assume all components are of size $s$ at most?}

Let any component C be of size |C| <= s. Find the smallest function h(s) so that : for all cascade c, Cost(c) < h(s)

\subsection{Cascade dependency on number of saturated edges}
